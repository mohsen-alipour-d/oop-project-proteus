...
Component* buildComponent(const vector<string>& t) {
    ...
    if (tag == "R")
        ...
        c = new ADC(name, x, y, stoi(t[7]), stod(t[8]));
    else if (tag == "DAC")
        c = new DAC(name, x, y, stoi(t[7]), stod(t[8]));
    else if (tag == "MCU") {
        Microcontroller* mcu = new Microcontroller(name, x, y, stod(t[7]));
        mcu->port(Microcontroller::PORT_A).setDirectionMask(static_cast<uint8_t>(stoi(t[8])));
        mcu->port(Microcontroller::PORT_B).setDirectionMask(static_cast<uint8_t>(stoi(t[9])));
        if (!mcu->flash().importHex(t[10])) {
            delete mcu;
            return nullptr;
        }
        c = mcu;
    } else if (tag == "MEM") {
        ExternalMemory* mem = new ExternalMemory(name, x, y, stoi(t[7]));
        if (!mem->importMemoryHex(t[8])) {
            delete mem;
            return nullptr;
        }
        c = mem;
    } else if (tag == "LCD")
        c = new LCD16x2(name, x, y);
    else if (tag == "KEYPAD")
        c = new MatrixKeypad(name, x, y);
