...
شبیه‌سازی را به Backend وصل کند. نسخهٔ یکپارچه با ۱۰۵ تست Backend و ۵۰ تست
...
1. قطعات دیجیتال/زمان‌دار با `comp->step(dt, simTime)` به‌روزرسانی می‌شوند.
2. `circuit.propagateVoltages()` Driverهای دیجیتال را روی Net پخش می‌کند.
3. `AnalogSolver::solve()` منابع و R/C/L/LED/Switch/Meterها را با MNA حل می‌کند.
4. چند پاس settle با `dt=0` اجرا می‌شود؛ حالت C/L فقط در پاس اول جلو می‌رود.
5. اسیلوسکوپ از ولتاژ حل‌شدهٔ Net نمونه می‌گیرد.

برای UI از `wireViews()` استفاده کنید؛ علاوه بر ولتاژ و `voltageResolved`، وضعیت یکی از
...
- خروجی‌های دیجیتال Driver هستند؛ Battery، DC Source و Clock منبع
  دوترمیناله‌اند و رابطهٔ ولتاژشان توسط AnalogSolver اعمال می‌شود.
- قطب `-` منابع و پین GND Reference کلاک باید به مسیر برگشت/GND همان جزیره وصل شوند.
